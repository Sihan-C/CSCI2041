(* This Fun server will support for private message by typing /p and the 
	person you want to chat like /p peter *)
open Lwt
...
module Server = struct
    ...
    let private_mesg sender priv_usr inp outp =
       Lwt_io.write outp "Enter private message: " >>= 
		fun () -> Lwt_io.read_line inp >>= 
		fun s -> Lwt_list.iter_p (fun (a,b) -> if a = priv_usr  
				then Lwt_io.write_line b ("<private message> "^sender^": "^s) 
    ...
    let chat_handler (inp,outp) =
      ...
      let handle_input l =  match Str.string_before l 2 with
		...
		| "/p" -> let x = Str.string_after l 3 in private_mesg !nick x inp outp		
...
(* TA COMMENT(hoppernj): 8/10 no code re-use, small delta *)